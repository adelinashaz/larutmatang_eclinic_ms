...
// for delete record treatment
if(isset($_GET['treatmentrecordid']))
{
	$updatesql ="UPDATE treatment_records SET status ='Deleted' WHERE treatment_records_id='$_GET[treatmentrecordid]' ";
	$qsql=mysqli_query($con,$updatesql);
	echo mysqli_error($con);
	if(mysqli_affected_rows($con) == 1)
	{
		echo "<script>alert('Appointment record deleted successfully');</script>";
		echo "<script>window.location='treatmentrecord.php?patientid=$_GET[patientid]&appointmentid=$_GET[appointmentid]';</script>";
	} else {
		echo "failed deleted..";
	}
}
  ...
  <div class="card" style="padding: 10px">
    <form method="post" action="" name="frmtreatrec" onSubmit="return validateform()" enctype="multipart/form-data">
    	<table class="table table-bordered table-striped">
      	<tbody>
        	...
        	<!-- for pass value appointment id usage-->
        	<input class="form-control" type="hidden" readonly name="select2" value='<?php echo "$_GET[appointmentid]"; ?>'/>
        	...
        	<?php
        		...
        		<tr>
          		<td colspan="2" align="center"><input class="form-control" type="submit" name="submit" id="submit" value="Submit" /> | <a href='patientreport.php?patientid=<?php echo "$_GET[patientid]"; ?>&appointmentid=<?php echo "$_GET[appointmentid]"; ?>'><strong>View Patient Report >></strong></a></td>
        		</tr>
      	</tbody>
    	</table>
    </form>
    <p>&nbsp;</p>
      <table class="table table-bordered table-striped">
          ...
          <?php
          // display treatment records list.
						$sql ="SELECT * FROM treatment_records WHERE patientid='$_GET[patientid]' AND appointmentid='$_GET[appointmentid]' ORDER BY status ASC, treatment_date DESC, treatment_time DESC";
						$qsql = mysqli_query($con,$sql);
						...
						while($rs = mysqli_fetch_array($qsql))
						{
							...
							if ($rs['status'] == 'Active') {
								...
								<td>&nbsp;$rs[status]</td>
								<td>&nbsp;
								<a href='treatmentrecord.php?treatmentrecordid=$rs[treatment_records_id]&patientid=$_GET[patientid]&appointmentid=$_GET[appointmentid]'>Delete</a> </td>
								</tr>";
							} else {
								echo "<tr>
								<td>&nbsp;$rstreatment[treatmenttype]</td>
