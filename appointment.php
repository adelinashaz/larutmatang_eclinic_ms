                                    ...
                                    <div class="form-line">
                                        ...
                                        ?>
                                                <select name="select4" id="select4" class=" form-control show-tick">
                                                    ...
                                                    <?php
                                                        $sqlpatients= "SELECT * FROM patient WHERE status='Active'";
                                                        $qsqlpatients = mysqli_query($con,$sqlpatients);
                                                        while($rspatients=mysqli_fetch_array($qsqlpatients))
                                                        {
                                                            if($rspatients['patientid'] == $rsedit['patientid'])
                                                            {
                                                                echo "<option/> Patient if";
                                                                // echo "<option value='$rspatient['patientid']' selected>$rspatient['patientid'] - $rspatient['patientname']</option>";
                                                            }
                                                            else
                                                            {
                                                                // echo "<option/> Patient else";
                                                                echo "<option value='$rspatients[patientid]' style='text-transform:uppercase'>$rspatients[patientname]</option>";
                                                            }
                                                        }
                                                    ?>
                                                </select>
                                    ...
                                    <div class="form-line">
                                        <select name="select5" id="select5" class=" form-control show-tick">
                                            ...
                                            <?php
                                                ...
                                                {
                                                    // if($rsdepartment['departmentid'] == $rsedit['departmentid']) // used when implement the edit apointment part.
                                                    if($rsdepartment['departmentid'] == null) // "== null" is just temporary, to come out the result in department.
                                                    {
                                                        echo "<option/> department if";
    ...
    if(isset($_POST['submit']))
    {
        ...
        else
        {
            ...
            if($qsql = mysqli_query($con,$sql))
            {
                ...
                echo "window.location='patientreport.php?patientid=$_POST[select4]';</script>";
            }
            else
            {
                echo mysqli_error($con);
