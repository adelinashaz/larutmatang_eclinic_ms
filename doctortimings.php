    ...
    <table class="table table-hover">
      <tbody>
        ...
        <?php
        	...
        	<td><select class="form-control"  name="selectday" id="selectday">
          <option value="">Select</option>
          <?php
		  			$arr = array("Monday","Tuesday", "Wednesday", "Thursday", "Friday", "Saturday", "Sunday");
		  			foreach($arr as $val)
		  			{
			   			if($val == $rsedit['available_day'])
			  			{
...
include("adfooter.php");
	...
	if(isset($_POST['submit']))
	{
		if(isset($_GET['editid']))
		{
			$sql ="UPDATE doctor_timings SET doctorid='$_POST[select2]', available_day = '$_POST[selectday]', start_time='$_POST[ftime]',end_time='$_POST[ttime]',status='$_POST[select]'  WHERE doctor_timings_id='$_GET[editid]'";

			if($qsql = mysqli_query($con,$sql))
			{
				echo "<script>alert('Doctor timings record updated successfully');";
				echo "window.location='viewdoctortimings.php';</script>";
			}
			else
			{
		...
		{
			$sql ="INSERT INTO doctor_timings(doctorid,available_day,start_time,end_time,status) values('$_POST[select2]', '$_POST[selectday]' , '$_POST[ftime]','$_POST[ttime]','$_POST[select]')";
			if($qsql = mysqli_query($con,$sql))
			{
				echo "<script>alert('Doctor timings record inserted successfully');";
				echo "window.location='viewdoctortimings.php';</script>";
			}
			else
			{
...
function validateform()
{
	...
	else if(document.frmdocttimings.selectday.value == "")
	{
		alert("Please select available day");
		document.frmdocttimings.selectday.focus();
		return false;
	}
