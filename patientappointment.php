                                ...
                                <form method="post" action="" name="frmpatapp" onSubmit="return validateform()"
                                    ...
                                    <ul class="row">
                                        ...
                                        ?>
                                            ...
                                            <li class="col-sm-6">
                                                <label><input placeholder="Postcode" type="number" class="form-control"
...
if(isset($_POST['submit']))
{
   ...
   else
   {
    if(isset($_SESSION['patientid']))
    {
        ...
        echo "<p class='text-center'> <a href='patientlogin.php'>View Appointment record</a>. </p>";            
    }
    else
    {
        echo "<h2 class='text-center'>Appointment taken successfully.. </h2>";
...
if(isset($_POST['submit']))
{
 ...
 else
 {
    $dt = date("Y-m-d");
    $tim = date("H:i:s");
    $sql ="INSERT INTO patient(patientname,admissiondate,admissiontime,address,city,mobileno,loginid,password,gender,dob,status,pincode) values('$_POST[patiente]','$dt','$tim','$_POST[textarea]','$_POST[city]','$_POST[mobileno]','$_POST[loginid]','$_POST[password]','$_POST[select6]','$_POST[dob]','Active', '$_POST[pincode]')";

    if($qsql = mysqli_query($con,$sql))
    {
        echo "<script>alert('Patient record inserted successfully');</script>"; 
        echo "window.location='index.php?patientid=$_GET[patientid]&appointmentid=$_GET[appointmentid]';</script>";
...
if(mysqli_num_rows($qsqlappointment) >= 1)
{
    ...
    echo "window.location='patientappointment.php?patientid=$_GET[patientid]&appointmentid=$_GET[appointmentid]';</script>";
}
else
{
