    ...
    <form method="post" action="" name="frmmedicine" onSubmit="return validateform()">
    	<table class="table table-hover">
      	<tbody>
        	...
        	<tr>
          	<td>Description</td>
          	<td><textarea placeholder="Enter Here" class="form-control no-resize" name="description" id="description" cols="45" rows="5" value="<?php /*echo $rsedit['description'];*/ ?>" /></textarea></td>
        	</tr>
        	<tr>
          	<td>Status</td>
...
<?php
	...
	if(isset($_POST['submit']))
	{
		...
		else
		{
			...
			if($qsql = mysqli_query($con,$sql))
			{
				echo "<script>alert('Medicine record inserted successfully');";
				echo "window.location.href='viewmedicine.php';</script>";
			}
			else
			{
