  	...
  	<div class="card" style="padding:10px">
          ...
          <?php
						...
						while($rs = mysqli_fetch_array($qsql))
						{
							...
							$sqlappoint = "SELECT * FROM appointment WHERE appointmentid='$rs[appointmentid]'";
							$qsqlappoint = mysqli_query($con,$sqlappoint);
							$rsappoint = mysqli_fetch_array($qsqlappoint);
    ...
    <?php
    	...
    	<table class="table table-striped">
      	<tbody>
        	...
        	<tr>
          	<td>Cost</td>
          	<td><input class="form-control" type="text" name="cost" id="cost" value="<?php 
          		if (empty($_POST['medicineid'])) {
          			echo "-- Please Select Medicine First --";
          			} else {
          				echo "$rsmedicine[medicinecost]";
          			}
          		?>" readonly style="background-color:whitesmoke;" /></td>
        	</tr>
        	<tr>
          	<td>Unit</td>
        	...
        	<tr>
          	<td>Total Cost</td>
          	<td><input class="form-control" type="text" name="totcost" id="totcost" value="<?php 
          		if (empty($_POST['medicineid'])) {
          			echo "-- Please Select Medicine First --";
          			} else {
          				echo "$rsedit[cost]";
  	...
  	<div class="card" style="padding:10px">
    	<table class="table table-hover table-striped">
      	<tbody>
			...
			<table>
				<tr><td>
	 				<center><a href='patientreport.php?patientid=<?php echo "$_GET[patientid]" ; ?>&appointmentid=<?php echo "$_GET[appointmentid]"; ?>'><strong>View Patient Report>></strong></a></center>
				</td></tr>
			</table>
...
if(isset($_POST['submit']))
{
	...
	else
	{
		...
		{	
			...
			echo "<script>window.location='prescriptionrecord.php?prescriptionid=$_GET[prescriptionid]&patientid=$_GET[patientid]&appointmentid=$_GET[appointmentid]';</script>";
		}
		else
		{
			echo mysqli_error($con);
